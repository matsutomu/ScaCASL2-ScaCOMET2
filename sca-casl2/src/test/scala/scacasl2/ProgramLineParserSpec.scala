...
class ProgramLineParserSpec extends FlatSpec with DiagrammedAssertions {
  ...
  it should " parse sample instructions (Equal Constatnts Replace Error Constants  ) " in {
    ...
    val errorVal = Array.fill(2001)("A").mkString("")
    val programLine = s"         OUT     ='$errorVal', =5"

    val result = ProgramLineParser.parseLine(programLine, 0)
    val innerResult = InnerParseResult.default().copy(lineNumber = Int.MaxValue - 1).parseEachLine(result.right.get)
    
    assert(innerResult === 
      InnerParseResult(2147483647,List.empty,Map.empty,
